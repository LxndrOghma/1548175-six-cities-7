...
function RatingInput({comment, setComment, starsCount, isChecked}) {
  return (
    <>
      <input className="form__rating-input visually-hidden" name="rating" value={starsCount} id={`${starsCount}-stars`} type="radio" onClick={(evt) => setComment({...comment, rating: +evt.target.value})} checked={isChecked}/>
